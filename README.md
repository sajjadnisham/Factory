...
4. Migrations run automatically before each release
   (`preDeployCommand: npx prisma migrate deploy`).
5. Open a shell on the service and create an admin user:
   ```bash
   ADMIN_INITIAL_USERNAME=admin ADMIN_INITIAL_PASSWORD='<a long password>' \
     npx tsx scripts/create-admin.ts
   npx tsx scripts/sync-stock.ts
   ```
   Or, to load the full demo catalogue and order history instead:
   ```bash
...
### One thing will block a live checkout

The app **refuses to start in production with `OTP_PROVIDER="console"`**, because
that provider prints verification codes to the server log. Until a real SMS
gateway is configured, customers cannot complete the phone verification step, so
they cannot place an order. Browsing, search, the cart and the admin panel all
work without it.

Configure a Maldivian SMS gateway and set:
